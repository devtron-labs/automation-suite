...
// TestClassA1SaveConfigMap todo need to take call for some changes once product will final that config file should delete after app deletion or not
// TestClassA1SaveConfigMap todo once product will final that we can add config file before deployment template of not
func (suite *ConfigsMapRouterTestSuite) TestClassA1SaveConfigMap() {
	...
	suite.Run("B=1=AddNewExternalConfigmapAsDataVolume", func() {
		newConfigName := strings.ToLower(Base.GetRandomStringOfGivenLength(6))
		requestPayloadForSaveAppCiPipeline := getRequestPayloadForSaveConfigmap(configId, newConfigName, createAppApiResponse.Id, "volume", true, true, true, true)
		byteValueOfSaveAppCiPipeline, _ := json.Marshal(requestPayloadForSaveAppCiPipeline)
		log.Println("=== Hitting the SaveConfigMap API ====")
		configMap := HitSaveConfigMap(byteValueOfSaveAppCiPipeline, suite.authToken)
		assert.True(suite.T(), configMap.Result.ConfigData[0].External)
