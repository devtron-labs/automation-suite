...
func (suite *PipelinesConfigRouterTestSuite) TestClassD2GetCiPipelineMaterial() {
	...
	suite.Run("A=1=GetCiPipelineMaterialWithValidMaterialId", func() {
		pipelineMaterial := HitGetCiPipelineMaterial(workflowResponse.CiPipelines[0].Id, suite.authToken)
		assert.Equal(suite.T(), config.GitHubProjectUrl, pipelineMaterial.Result[0].GitMaterialUrl)
		assert.Equal(suite.T(), "main", pipelineMaterial.Result[0].Value)
		assert.Equal(suite.T(), "SOURCE_TYPE_BRANCH_FIXED", pipelineMaterial.Result[0].Type)
		assert.NotNil(suite.T(), pipelineMaterial.Result[0].History)
		assert.NotNil(suite.T(), pipelineMaterial.Result[0].LastFetchTime)
	})

	suite.Run("A=2=GetCiPipelineMaterialWithInvalidMaterialId", func() {
		randomMaterialId := Base.GetRandomNumberOf9Digit()
		workflowStatus := HitGetCiPipelineMaterial(randomMaterialId, suite.authToken)
		assert.Equal(suite.T(), "pg: no rows in result set", workflowStatus.Errors[0].UserMessage)
	})
